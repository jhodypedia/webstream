...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!user) return res.render('admin/login', { title:'Admin Login', error: 'User not found', layout:false });

    const ok = await bcrypt.compare(password, user.password_hash || '');
    if (!ok) return res.render('admin/login', { title:'Admin Login', error: 'Invalid password', layout:false });
