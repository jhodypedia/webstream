// ===============================
// PansaStream Admin Panel Script
// ===============================

const socket = io();

// --- FIX: definisikan dulu fungsi page sebelum jQuery ready() ---
function initUploadPage() {
  if (window.Dropzone === undefined) {
    console.error("❌ Dropzone belum dimuat!");
    return;
  }
  Dropzone.autoDiscover = false;

  // Hapus instance lama biar tidak error "already attached"
  ...
  const dz = new Dropzone("#dzUpload", {
    ...
    dictDefaultMessage: `<i class='fa-solid fa-cloud-arrow-up fa-2x'></i>
    ...
    init: function () {
      this.on("success", (file, res) => {
        if (res && res.ok) {
          ...
          Swal.fire("Error", (res && res.error) || "Upload gagal", "error");
        }
      });
...
function initVideosPage() {
    ...
    $.get("/admin/api/videos")
      .done((res) => {
        ...
        const rows = (res.items || []).map((v) => {
          ...
          return [
            thumb,
            `<div><div style="font-weight:600">${escapeHtml(
              v.title || "-"
            )}</div><div style="color:#9aa7b6;font-size:12px">${v.slug}</div></div>`,
            v.status,
            v.views || 0,
            created,
            `
  ...
  $form.on("submit", function (e) {
    ...
    const req = d.id
      ? $.ajax({
          url: "/admin/api/videos/" + d.id,
          method: "PUT",
          contentType: "application/json",
          data: JSON.stringify(payload),
        })
      : $.ajax({
          url: "/admin/api/videos",
          method: "POST",
          contentType: "application/json",
          data: JSON.stringify(payload),
        });
  ...
  function loadJobs() {
    $.get("/admin/api/jobs").done((res) => {
      ...
      const rows = (res.items || []).map((j) => {
        const updated = new Date(j.updatedAt || j.createdAt).toLocaleString(
          "id-ID"
        );
...
function initSettingsPage() {
  $("#formSettings").on("submit", function (e) {
    ...
    fetch("/admin/api/settings", {
      ...
      .then((res) => {
        if (res.ok)
          Swal.fire("Saved", "Settings updated (runtime reloaded)", "success");
        else Swal.fire("Error", res.error || "Failed", "error");
      })
...
// ===============================
// Loader style (inject if missing)
// ===============================
